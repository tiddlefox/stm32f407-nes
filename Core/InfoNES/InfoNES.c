...
/* RAM (CCM to save SRAM1 for WorkFrame DMA) */
BYTE RAM[ RAM_SIZE ] __attribute__((section(".ccm")));

/* SRAM */
BYTE SRAM[ SRAM_SIZE ] __attribute__((section(".ccm")));
...
/* PPU RAM (CCM) */
BYTE PPURAM[ PPURAM_SIZE ] __attribute__((section(".ccm")));
...
/* Character Buffer */
BYTE ChrBuf[ 256 * 2 * 8 * 8 ] __attribute__((section(".ccm")));
 ...
 *
 *  v1 fix: Restore while(1) loop and remove duplicate InfoNES_Init()
 */
 ...
 */
  ...
  {    
    ...
    // HSYNC Wait (must be instant — per-scanline!)
    InfoNES_Wait();
 ...
 */
  ...
  /*  Render a scanline                                                */
  /*-------------------------------------------------------------------*/
  if ( PPU_ScanTable[ PPU_Scanline ] == SCAN_ON_SCREEN )
  {
    InfoNES_DrawLine();
  }
 ...
 */
  ...
  /* v1: Skip scanlines beyond WorkFrame to prevent buffer overflow.
   * NES_DISP_HEIGHT is reduced to save SRAM (120KB → 109KB). */
  if (PPU_Scanline >= NES_DISP_HEIGHT) return;
