 ...
 */
   ...
   * 6502 registers (PC,A,X,Y,SP,F) are global vars in SRAM — every opcode
   * forces memory load/store. Copy to locals in an inner block so the ARM
   * compiler keeps them in core registers (r4-r11). Macros expand inside
   * the block and see locals instead of globals. Pointers to globals are
   * saved in the outer scope for writeback. */
  WORD * const pPC = &PC;
  BYTE * const pA  = &A, * const pX  = &X, * const pY  = &Y;
  BYTE * const pSP = &SP, * const pF = &F;

  {  /* --- Inner block: locals shadow globals --- */
  WORD PC = *pPC;
  BYTE A  = *pA,  X  = *pX,  Y  = *pY;
  BYTE SP = *pSP, F  = *pF;

  /* Override K6502_ReadAbsX/Y/IY as macros so they see the local
   * PC/X/Y/wA0/wA1 instead of the stale globals. These are defined
   * here (before the switch) so preprocessing is in scope. */
  #undef  K6502_ReadAbsX
  #undef  K6502_ReadAbsY
  #undef  K6502_ReadIY
  #define K6502_ReadAbsX() ( wA0 = AA_ABS, wA1 = wA0 + X, \
      CLK( ( wA0 & 0x0100 ) != ( wA1 & 0x0100 ) ), K6502_Read( wA1 ) )
  #define K6502_ReadAbsY() ( wA0 = AA_ABS, wA1 = wA0 + Y, \
      CLK( ( wA0 & 0x0100 ) != ( wA1 & 0x0100 ) ), K6502_Read( wA1 ) )
  #define K6502_ReadIY() ( wA0 = K6502_ReadZpW( K6502_Read( PC++ ) ), \
      wA1 = wA0 + Y, \
      CLK( ( wA0 & 0x0100 ) != ( wA1 & 0x0100 ) ), K6502_Read( wA1 ) )
  ...
  /* Write back localized registers to globals via saved pointers */
  *pPC = PC;
  *pA = A; *pX = X; *pY = Y;
  *pSP = SP; *pF = F;
  ...
  #undef K6502_ReadAbsY
  #undef K6502_ReadIY
  }  /* end of inner block (register localization scope) */
}
